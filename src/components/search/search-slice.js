...
import { fetchPokemonsForType as fetchPokemonsForTypeUtil, fetchPokemonsForTypes } from '../../utils/pokemon-api';
...
export const searchSlice = createSlice({
    name: 'search',
    initialState: { pokemonsLoaded: false, searchExpression: '', type: '', caughtOnly: false, pokemons: [], caughtPokemons: [], pokemonsFiltered: [] },
    reducers: {
        updateFilter: (state, action) => {
            ...
            state.type = action.payload.type === undefined ? state.type : action.payload.type;
            ...
            const pokemonsFilteredByCaughtOnly = state.caughtOnly ? state.pokemons.filter(pokemon => {
                return new Set(state.caughtPokemons).has(pokemon);
            }) : state.pokemons;

            const pokemonsFilteredByType = state.type && state.type !== 'all' ? state.pokemons.filter(p => p.types.find(type => type.url === state.type)) : state.pokemons;

            let filtersIntersection = intersection(intersection(new Set(pokemonsFilteredBySearchExpression), new Set(pokemonsFilteredByType)), new Set(pokemonsFilteredByCaughtOnly));
